...
| `tentativas_consumidas` | Opcional; escrito só pela automação. Conta reprovações PERSISTIDAS no arquivo do ticket — incrementa e grava (no mesmo write que `notas_status`) a cada reprovação, sobrevivendo a reinício. Ausente = `0` (compat com ticket legado). Quando ultrapassa `max_retries` do `000-config.json`, a própria reprovação que estoura já grava `status: "bloqueado"`. A escolha do `retry_final_model` usa este valor persistido, não um contador em memória. |
