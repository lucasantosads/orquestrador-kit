...
| `contexto_juiz` | Opcional. Caminhos de REFERÊNCIA que o critério manda comparar e que não fazem parte do diff (porte do 622 do Actus). O harness os lê da BASE da tentativa e os entrega ao juiz rotulados; tetos de 400 linhas por arquivo e 1200 no total. Caminho que não existe na branch alvo ou que está na própria allowlist reprova no gate (check 9). |
| `criterios_aceite[].exemplos_regex` | Obrigatório quando o `cmd` passa padrão a `grep` com `-E` (`-E`, `-cE`, `-vcE`, `-ciE`, `egrep`, `-e` com `-E`) ou literal `/.../` a `awk`; `grep -F` e `grep` sem `-E` não exigem. Lista de `{ regex, positivo, negativo, flags?, ferramenta? }`: `regex` é o padrão EXATAMENTE como está no `cmd` (depois do parse do JSON); `flags` são as opções de grep que mudam o casamento (padrão `-E`); `ferramenta` é `grep` (padrão) ou `awk`. O gate de ticket (check 10, porte do 624 do Actus) prova em `/usr/bin/grep` / `/usr/bin/awk` por argv e reprova: padrão sem exemplo; regex que não aparece literal no `cmd`; positivo que não casa; negativo que casa; `\[` ou `\]` dentro de classe `[...]` (no grep BSD o `]` fecha a classe — o `src/A-]:1` fabricado do 503 do Actus). Binário ausente ou timeout = o gate não rodou (rc 2), nunca "passou". Exemplo: `{"regex": "(src\|scripts)/[A-Za-z0-9_./()-]+:[0-9]+", "positivo": "src/app/route.ts:68", "negativo": "src/A-]:1"}`. |
